...
async function main() {
    ...
    let tip3Contract = await contracts.locklift.factory.getContract('TONTokenWallet', configuration.buildDirectory);
    let messages = await contracts.locklift.ton.client.net.query_collection({
        collection: 'messages',
        filter: {
            dst: { eq: contracts.userAccountContract.address },
        },
        order: [{
            path: 'created_lt',
            direction: 'ASC'
    ...
    for (let message of messages.result) {
        try {
            let result = await contracts.locklift.ton.client.abi.decode_message_body({
                ...
                abi: {
                    type: 'Contract',
                    value: contracts.userAccountContract.abi
                }
            });
            console.log(result);
        } catch (err) {
